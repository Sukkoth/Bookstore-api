...
export const prismaQueryGenertor = (filterArray, modelName) => {
  if (!filterArray || !modelName) {
    throw new AppError({
      messag: "You need to provide both filterArray and modelName to parameter",
      statusCode: 400,
    });
  }
  ...
  filterArray.forEach((element) => {
    ...
    if (element.field.includes(".")) {
      console.log("found nested relation");
      ...
      splittedArray.forEach((relation, index) => {
        if (!testerRelation) {
          console.log("No tester", relation);
          if (
          ...
          console.log(
            "elsing",
            relation,
            Object.keys(schema.definitions[testerRelation].properties)
          );
          if (
            !Object.keys(
          ...
          console.log("Relation in else", relation);
          ...
          if (splittedArray.length - 1 === index) {
            ...
            console.log("Ref", "RelationName", testerRelation, found);
